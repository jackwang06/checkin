      ...
      {!report && hits.map((s) => (
        <View key={s.id} className='card row' style={{ justifyContent: 'space-between' }} onClick={() => openReport(s.id)}>
          <View className='row' style={{ gap: '12rpx' }}>
            <Text>{s.name}</Text>
            <Text className='faint' style={{ fontSize: '26rpx' }}>{s.id}</Text>
          </View>
          <Text className='muted' style={{ fontSize: '28rpx' }}>{s.className}</Text>
        </View>
      ))}
          ...
          <View className='card'>
            <View className='row' style={{ justifyContent: 'space-between' }}>
              <View>
                ...
                <Text className='muted' style={{ display: 'block', marginTop: '6rpx', fontSize: '28rpx' }}>
                  {report.student.grade}级 {report.student.major} · {report.student.className}
                </Text>
              </View>
              <Text className='btn-outline' style={{ padding: '12rpx 22rpx', fontSize: '28rpx' }}
            ...
            {report.grid.length === 0 ? <Text className='muted'>无考勤记录</Text> : (
              <View>
                <View className='row' style={{ borderBottom: '1px solid var(--border)', paddingBottom: '10rpx' }}>
                  <Text className='faint' style={{ flex: '0 0 90rpx', fontSize: '28rpx' }}>周次</Text>
                  {cols.map((c) => <Text key={c.key} className='faint' style={{ flex: 1, textAlign: 'center', fontSize: '28rpx' }}>{c.label}</Text>)}
                </View>
                {[...report.grid].sort((a, b) => a.term === b.term ? b.weekNo - a.weekNo : (a.term < b.term ? 1 : -1)).map((row) => (
                  <View key={`${row.term}-${row.weekNo}`} className='row' style={{ padding: '12rpx 0', borderBottom: '1px solid var(--border)' }}>
                    <Text className='muted' style={{ flex: '0 0 90rpx', fontSize: '28rpx' }}>{row.weekNo}周</Text>
                    {cols.map((c) => (
